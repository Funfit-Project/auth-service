    ...
    public void publishOutboxMessage() throws JsonProcessingException {
        ...
        for (Outbox outbox : outboxes) {
            if (outbox.getTopic().equals("user-info-updated")) {
                kafkaProducerService.publishUserInfoUpdated(outbox.getPayload());
            }
            if (outbox.getTopic().equals("pt-member-joined")) {
                kafkaProducerService.publishPtMemberJoined(objectMapper.readValue(outbox.getPayload(), PtMemberJoinedDto.class));
            }
